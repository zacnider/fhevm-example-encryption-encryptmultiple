...
import { EntropyEncryptMultiple } from "../types";

/**
 * @title EntropyEncryptMultiple Tests
 * @notice Comprehensive tests for EntropyEncryptMultiple contract with EntropyOracle integration
 * @chapter encryption
 */
describe("EntropyEncryptMultiple", function () {
    ...
    // Deploy EntropyEncryptMultiple
    const ContractFactory = await hre.ethers.getContractFactory("EntropyEncryptMultiple");
    const contract = await ContractFactory.deploy(oracleAddress) as any;
    await contract.waitForDeployment();
    const contractAddress = await contract.getAddress();
    
    await hre.fhevm.assertCoprocessorInitialized(contract, "EntropyEncryptMultiple");
  ...
  describe("Basic Encryption - Single Value", function () {
    it("Should encrypt and store single value", async function () {
      ...
      await contract.encryptAndStore(0, encryptedInput.handles[0], encryptedInput.inputProof);
      
      expect(await contract.isKeyInitialized(0)).to.be.true;
      expect(await contract.getTotalValues()).to.equal(1);
    });

    it("Should encrypt and store multiple single values", async function () {
      const { contract, contractAddress, owner } = await loadFixture(deployContractFixture);
      ...
      await contract.encryptAndStore(0, encryptedInput1.handles[0], encryptedInput1.inputProof);
      ...
      await contract.encryptAndStore(1, encryptedInput2.handles[0], encryptedInput2.inputProof);
      
      expect(await contract.isKeyInitialized(0)).to.be.true;
      expect(await contract.isKeyInitialized(1)).to.be.true;
      expect(await contract.getTotalValues()).to.equal(2);
    });
  });

  describe("Batch Encryption", function () {
    it("Should encrypt and store multiple values in batch", async function () {
      const { contract, contractAddress, owner } = await loadFixture(deployContractFixture);
      ...
      await contract.encryptAndStoreBatch(keys, encryptedInputs, inputProofs);
    ...
    it("Should fail with mismatched array lengths", async function () {
      ...
      await expect(
        contract.encryptAndStoreBatch(keys, [encryptedInput.handles[0]], [encryptedInput.inputProof])
  ...
  describe("Entropy-Enhanced Encryption", function () {
    it("Should request entropy", async function () {
      ...
      const tag = hre.ethers.id("test-encryption-multiple");
    ...
    it("Should encrypt single value with entropy", async function () {
      const { contract, contractAddress, owner, oracle } = await loadFixture(deployContractFixture);
      
      // Request entropy
      const tag = hre.ethers.id("encrypt-multiple-1");
      const fee = await oracle.getFee();
      ...
      if (requestId) {
        // Wait a bit for entropy (in real scenario, wait for fulfillment)
  ...
  describe("Batch Entropy-Enhanced Encryption", function () {
    it("Should encrypt multiple values with entropy in batch", async function () {
      const { contract, contractAddress, owner, oracle } = await loadFixture(deployContractFixture);
      
      // Request entropy
      const tag = hre.ethers.id("encrypt-batch-entropy");
      const fee = await oracle.getFee();
      const tx = await contract.requestEntropy(tag, { value: fee });
      const receipt = await tx.wait();
      const event = receipt?.logs.find((log: any) => {
  ...
  describe("Value Retrieval", function () {
    it("Should retrieve encrypted value by key", async function () {
      ...
      await contract.encryptAndStore(5, encryptedInput.handles[0], encryptedInput.inputProof);
